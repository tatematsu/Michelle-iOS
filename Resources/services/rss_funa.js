...
var RSS_URL = 'http://funakoshi.la-michelle.com/feed/'; // feed URL
...
exports.loadRssFeed = function(o, tries) {
	...
	xhr.onload = function(e) {
		...
		for (var i = 0; i < items.length; i++) {
			...
			var image;
			try {
			var image = item.getElementsByTagNameNS('http://funakoshi.la-michelle.com/', 'thumbnail').item(0).getElementsByTagName('img').item(0).getAttribute('src');
			} catch (e) {
				image = '';
			}
